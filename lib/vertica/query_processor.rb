  ...
  def process_message(message)
  	case message
  	  when Vertica::Protocol::ErrorResponse
    		self.error = Vertica::Error::QueryError.from_error_response(message, sql)
  	  when Vertica::Protocol::EmptyQueryResponse
    		self.error = Vertica::Error::EmptyQueryError.new("A SQL string was expected, but the given string was blank or only contained SQL comments.")
